...
1. The name has no prior meaning on an `--internal` bridge to shadow. Measured: a container on such a bridge cannot resolve `host.docker.internal` at all — Docker does not inject it there — so it resolves only because the relay attaches it as an alias.
2. A second accepted spelling would silently skip `model-probe.ts:hostFacingBaseUrl`'s host-side rewrite and misprobe, which is a new quiet failure of exactly the kind this subsystem exists to refuse.

Anything needing the *actual* Docker host from inside the bridge must use the gateway address (§12.8), never this name — which was already true before this amendment. Tracked as ISC-264.
