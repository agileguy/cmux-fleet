...
| `<run-dir>/replies/<worker>` | `/replies` | **ro** | always | the REPLY PLANE: one `<child-task-id>.json` per child task, holding that child's harvested result, for a collator that fanned work out and has no wire to be answered on. The inbound half of the exchange whose outbound half is `/outbox/<task-id>/dispatch-request.json` — and `ro` because the two halves must not share a permission: a reply delivered into the worker's own writable outbox is evidence its subject can edit before quoting it. Files are 0444, the directory 0755, and each file is rewritten IN PLACE, never tmp+rename, because a bind mount pins the inode. In `docker/verbgate`'s integrity check — BESIDE the file loop rather than in it (`docker/verbgate:284-286`), because the loop tests each member's PARENT too, which is safe for a file under `/policy` and meaningless for a directory whose dirname is `/`; corrected 2026-09-08, when ISC-1092 made `/policy/replies` a genuine fourth loop member one word away in the same script — so a dropped `:ro` costs the whole worker rather than one forged reply — `SRD-REVIEW-CONSOLE` §6.4, D6 |
...
| `<run-dir>/workers/<worker>/replies-policy` | `/policy/replies` | **ro** | always | the DECLARED REPLY SET: which of the files in `/replies` belong to THIS turn, as `{ "schema": "pifleet.replies/v1", "task_id", "replies": [{ "task_id", "worker", "aspect", "path" }] }`. Declared and not discovered, because `/replies` is one directory per worker per **run** and a standing console publishes into it every sweep — so a tool that listed it would hand sweep 5's collator sweeps 1 through 5, each reading like a good answer to the question actually asked. An EMPTY array is a value: it is what lets the reader say "nothing was declared" rather than "the directory is empty". Written by whatever publishes the replies, in the same act, so the set the worker can read is by construction the set the host published. A sibling of `/policy/task` and `/policy/dispatch` on the same surface and under the same integrity rule — rewritten IN PLACE, never tmp+rename, because a bind mount pins the inode — `SRD-WORKER-DISPATCH-EXTENSION` §7.4, D6, 9.6 |
| `<run-dir>/workers/<worker>/secrets` | `/secrets` | **ro** | always | *(CORRECTED 2026-09-08: this column read "only when the worker was granted ≥1 `secrets:` name" and had been stale since D8. `src/config/render.ts:637` records the gate being REMOVED rather than widened, and ISC-407/ISC-408 rest on that. `docs-currency.test.ts` checks the PATH column only, so nothing caught it.)* granted credentials, one file per name at 0444, reached through `<NAME>_FILE` — §12.4 |
...
> 3. `git -C <repo> fetch worker-<id> <branch>` brings the worker's commits into the operator's own checkout, and `mergeWorkerBranch` in `src/run/pm-integration.ts` lands them on an integration branch. That is a library function the orchestrator calls — there is no `pifleet merge` verb — so the merge is gated inside the function, not left to a bare `git merge --no-ff` an operator runs by hand.
>
> **The merge is the first outbound path that lands worker-authored files in the operator's own working tree** — the harvest/outbox path (§8) is outbound too and predates it, but it lands files in the run directory's harvested outbox, not in the checkout. Isolation was only ever asked to protect the inside — a worker from a hostile repository, via `neutralizeRepoHazards` on the way in. The fetch-and-merge path is the first time worker-authored files move into the checkout read by the process holding the operator's git identity and credentials, so the merge is gated: fetch freely, inspect the incoming tree before materialising it, merge with hooks disabled, and re-scan the operator's checkout after every merge. Hooks are genuinely suppressed (`core.hooksPath=/dev/null`); a `filter=` driver arriving in a worker's diff is refused by the inspect step before it can run; and a filter driver already resident in the operator's own checkout is NOT covered by either flag — `core.attributesFile=/dev/null` names an additional global attributes file and was never the switch for the tracked `.gitattributes`, so a resident driver still runs during the merge's checkout. The gate is specified in `SRD-FLEET-PM-001` §6.2.1 and implemented in `src/run/pm-integration.ts`'s `mergeWorkerBranch`.
