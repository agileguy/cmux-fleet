...
| **Q12** | **PARTIALLY ANSWERED 2026-09-05 by the phase 6 dogfood, and the count is 2 of 2, not the 3 the probe asks for.** Both rounds this run driven from the calling session — the phases 1-2 round and phase 6's — ended in a collation the session read. Neither poll ended in anything else. That is evidence for the automation and it is not yet the probe: two is not three, and both were run by the same operator on the same machine within two hours. **What the run did settle is the shape of the wait**: the collation appeared 4-5 minutes after dispatch in both rounds, and the session polled the run tree (`relay/col-1/<parent>.json` and `replies/col-1/*`) rather than a verb, exactly as this row predicts. The missing timeout is still missing. Should the review round be driven by the calling session at all, or is it a human step? §6.5's step table shows only steps 1-2 and 8 are the session's; steps 3-7 are the collator and the relay, there is no "wait for the review" verb, and a session polling for the collate artifact has no timeout of its own | Run three review rounds and record how often the session's poll ends in something other than a collation. **If the answer is "never", the automation is fine; if it is "sometimes", the failure is silent and expensive** | Nothing structurally. It decides whether §8.2's step 6-7 is a loop or an instruction to the operator |
...
| **Q3** | **PARTIALLY ANSWERED 2026-09-05, and by accident rather than by probe: the calling session was COMPACTED twice during this run**, once between phases 4 and 5 and once during phase 5. Both times the run continued from `.claude/project-manager-state.json` plus the run tree, and no phase was repeated or skipped. **That is not the probe** — a compaction is not a kill, the orchestrator process survived both, and no engineer was mid-turn on either occasion. What it does show is that the state file carried enough to resume a run whose conversation had been summarised away, which is the weaker half of what §6.6 claims. The kill-with-two-engineers-running case is untaken. What does a ProjectManager run do when the calling session is compacted or lost mid-phase? §6.6 makes recovery derivable and §7.6 makes the partition recoverable, but nothing *restarts* the orchestrator the way `./scripts/review` restarts the relay | Kill a session mid-phase with two engineers running, then resume from the state file and confirm §6.6's five-step recovery reaches the same integration branch. **This is the honest test of D1's cost** | **Nothing in the design.** It bounds how much of a phase an interruption costs |
...
| **Q5** | **ONE DATA POINT, 2026-09-05.** The dogfood's own phase partitioned to ONE engineer and left three seats idle — not because the tasks overlapped, but because only one dispatchable task remained. It did not conflict. That is 1 of 1 serialised for a reason this row did not anticipate: **a phase can serialise for want of work rather than for want of disjointness**, and the idle-seat cost is identical either way. The count this row wants still needs five phases with real parallel work. Is file-disjoint partitioning worth its idle seats, and should an overlap be detected before the merge? §6.3, D4. A post-hoc check comparing the two workers' `files_changed` arrays would catch an overlap one step earlier than git — after the tokens are spent, before the merge | Run five phases and record: how many partitioned disjointly, how many serialised, how many conflicted anyway. **Under ten phases this is a measurement, not a study** | **Nothing.** D4 is recorded as recommended-and-unmeasured, which is what this row exists to say |
...
| **Q7** | **MEASURED 2026-09-05, one phase.** Both consoles stayed up for the whole run. The four review seats were idle for the engineer's ~5 minutes and again for the fix round's ~2, and generated nothing in either window — `transcript_activity.entries` did not move on any of the three lenses between rounds. So the idle cost is a rate limit and a bill and not a token spend, which is the half of this row that was answerable without the vendor's billing model. **The recreate cost is now measured too**: repointing the development console stopped three runs and rebuilt four panes, and the four `up`s were complete inside one poll interval — far cheaper than §0.5 correction 2 implies, because the seats came up warm. Do both consoles stay up for the whole run, or is the review console brought up per phase? §1.3, §6.4. Eight hosted seats standing idle between phases is a rate limit and a bill; bringing the review console up per phase is a `--recreate` per phase, and §0.5 correction 2 is why that is not free | Measure the idle cost of four review seats over one phase's engineer-and-tester time. **If the seats generate nothing while idle the question answers itself**; the vendor's billing model decides it | **Nothing structurally.** It is an operating decision recorded so it is made rather than defaulted |
