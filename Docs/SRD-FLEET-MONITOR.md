...
there is no blank window — **Q10 asked whether it nonetheless flashes in a real pane and the
answer, observed 2026-09-03 against a `clear`-based control, is NO**), and a one-time
...
| **Q3** | **ANSWERED 2026-09-03, and §3.5's arithmetic was WRONG BY A WIDE MARGIN IN THE SAFE DIRECTION.** Measured on the operator's own operations console after the pane merge (D1), by reading the monitor pane's pty: **171 columns × 32 rows**. §3.5 reasoned from `OPERATIONS_TOP_FRACTION = 2/3` to "roughly 80×10 for one cell" — so the real pane is over **2× wider and 3× taller** than the estimate the degradation ladder was designed against. The estimate was arithmetic on an unmeasured window size, exactly as the question said. **The second half — what becomes unreadable first — was closed by DERIVATION rather than by this measurement, and that ordering matters:** ISC-485 sets the floor at 32 columns from the cells §6.5 forbids dropping, which is a property of the design rather than of whichever terminal happened to be open. A floor measured from this pane would have gone stale on the next resize, and at 171 columns it would also have been measured somewhere the ladder never engages. The measurement's real value is the opposite of what the question expected: **the console pane is nowhere near the floor**, so §6.5's ladder is insurance rather than a routine code path, and no criterion should be graded on the assumption that an operator sees it. ~~Original question:~~ **What is the actual pane geometry, and what becomes unreadable first?** §3.5 reasons from `OPERATIONS_TOP_FRACTION = 2/3` to "roughly 80×10", and that is arithmetic on an unmeasured window size. | Open the operations console, run `stty size` in the bottom-left pane, then again after merging the bottom row into one pane, and shrink the terminal until the fleet row loses its activity column. | **§6.5's degradation order and D14's floor.** Both are currently stated as orderings with no numbers, which is exactly the shape that gets discovered at grading time. |
| **Q4** | **ANSWERED 2026-09-03: YES, SIGWINCH IS DELIVERED. The expectation was right and the first probe was DEFECTIVE, not informative.** Measured in a throwaway cmux window whose workspace was started the same way the monitor is — `--command` text typed into the pane's shell, so the process is a foreground job of that shell. Three pty resizes driven by splits and a close: **209x60 -> 104x60 -> 104x29 -> 209x29. Four signals against three size changes, and every size change was PRECEDED by its signal** (the first by 1 ms, measured by a poller that watched the pty independently of the handler). The duplicate at +28228/+28255 ms is a layout settling, not a miss. **The second probe differs from the first in one respect and it is the whole difference: the window was FOCUSED BEFORE the layout changed.** cmux does not re-lay-out an unrendered window, so the first attempt's pane never actually resized — and zero signals against zero resizes measures nothing, which is why it was recorded as inconclusive rather than negative. This probe therefore polls the pty's own dimensions alongside the handler, so "no resize happened" can never again masquerade as "no signal arrived". **Nothing in the design changes:** ISC-497 already made the frame follow the pane by re-reading `process.stdout.columns` every paint, and that stays the mechanism. The signal is now confirmed as what it was built to be — an optimisation that removes up to one interval of lag. ~~Original question:~~ One probe was attempted and was INCONCLUSIVE rather than negative: a program started through cmux `--command` injection in a throwaway window recorded zero signals, but `stty` showed its pty never actually changed size, so zero measured nothing. **The design stopped needing the answer instead.** The monitor now re-reads `process.stdout.columns` on every paint, so the frame follows the pane whichever way Q4 lands; the `resize` listener is kept as an optimisation that removes the up-to-one-interval lag, not as the mechanism. This row called polling "a different and worse design", and that holds only when polling is ADDED to a loop that lacks one — this loop already repaints on an interval. A design that needed Q4's answer would carry a dependency on a terminal's behaviour, which is exactly what ISC-491 avoids. ~~Original question:~~ **Does a program started by cmux's shell injection receive `SIGWINCH`?** `--command` text is typed into the pane's shell (`operations-plan.ts:51-54`), so the monitor is a foreground job of that shell rather than a process cmux spawned directly. The expectation is yes — that is ordinary job control — but this repository has been wrong before about what a pane delivers, and §1.1's `less`-at-`(END)` finding is the precedent for measuring rather than assuming. | Start the monitor in an operations pane, resize the cmux window, and see whether it repaints at the new size. | **D2's resize handling.** If no signal arrives, the renderer must poll `process.stdout.columns`, which is a different and worse design. |
