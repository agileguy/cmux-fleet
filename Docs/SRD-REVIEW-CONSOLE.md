...
| **Q6** | Is `partial` the right verdict for a two-of-three review, or is it the wrong axis? `partial` in this fleet has meant "the work was partly done". A two-lens review is *complete work with a missing lens*. | Decide whether the record needs a `coverage` datum beside the verdict rather than folding coverage into it | **§6.6's table.** The lattice mapping works either way; what changes is whether an operator reading `partial` learns the right thing |
