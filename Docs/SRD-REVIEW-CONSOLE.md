...
| **Q6** | **ANSWERED 2026-09-05 by the owner — SPLIT THE AXES.** Coverage becomes its own datum BESIDE the verdict rather than being folded into it: the record says how many lenses reported *and* what the findings say, and neither word stands in for the other. **The collision this removes is real and not cosmetic** — today a COMPLETE review of shaky code and a BROKEN review of sound code both read `partial`, so the single word an operator scans first is the one word that cannot separate *the console failed* from *the code has problems*. That is the same confusion §6.8's grading rules spend their length avoiding one layer down, arriving at the layer an operator actually reads. **IMPLEMENTED 2026-09-05.** `relay.ts`'s `claim: "success" | "partial"` is replaced on both outcome arms by `coverage: {reported, dispatched}`, counted by the HOST from what it dispatched and what it harvested — never asked of the model, because coverage was never the worker's to claim. The brief now states coverage as a fact and instructs status as a RULE rather than handing back a value: `success` when the collator has faithfully collated what reached it *including when a lens is missing*, `partial` only when its own collation is incomplete. `roles/collator.md`'s *"Claim the status the brief tells you to claim"* is replaced by the same rule. **A second defect was found while implementing it, and it is the stronger argument for the split:** `censusCeiling` declines on any claim that is not `success` (`collation-census.ts:490`), and the old instruction told the collator to write `partial` whenever a lens went missing — so **the structural quality check was switched OFF for exactly the reviews most likely to be thin**, the ones that had just lost a lens. Separating the axes is what lets that ceiling engage where it was always meant to. Three valid mutations confirm the probes bite: dropping the anti-restatement rule, reintroducing a coverage-derived `status: "partial"`, and dropping the coverage line each turn the suite red. **The original question follows.** Is `partial` the right verdict for a two-of-three review, or is it the wrong axis? `partial` in this fleet has meant "the work was partly done". A two-lens review is *complete work with a missing lens*. | Decide whether the record needs a `coverage` datum beside the verdict rather than folding coverage into it | **§6.6's table.** The lattice mapping works either way; what changes is whether an operator reading `partial` learns the right thing |
...
- Two of three children succeeding produces a collation dispatched with the missing aspects named, and
  coverage carried BESIDE the collator's status rather than folded into it (§9 Q6). *Probe: fail one
  child deliberately; assert the brief names the missing lens, assert it carries `COVERAGE: 2 of 3
  lenses reported`, and assert it does NOT hand back `status: "partial"`. The last arm is the one that
  would have caught the old shape: handing the count back as a status ALSO disabled `censusCeiling`,
  which declines on any claim short of `success`, so the structural check went quiet on precisely the
  review that had just lost a lens.*
