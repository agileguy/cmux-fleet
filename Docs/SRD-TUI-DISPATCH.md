...
- **Auto-triggering a staged task on the operator's behalf** — the one design that would close the
  ...
  §9 Q4 states the probe. **UPDATE 2026-09-02 — Q4 was probed and the answer is YES**, so this is no
  longer deferred for want of a mechanism. `pi.sendUserMessage()` starts a turn from inside the
  container with nothing typed, and the brief never enters the composer's buffer. What is deferred
  now is a decision, not a discovery: whether an operator sitting at an adopted terminal should have
  a turn begin under them unannounced. That is the owner's call, and §4.3's hazard argument does not
  settle it — §4.3 was about a brief being *concatenated onto* what the operator was typing, and this
...
| **Q4** | ~~Is there a **container-side** trigger that starts a Pi turn without writing to the surface?~~ **ANSWERED 2026-09-02: YES.** Pi 0.79.6 enumerates its own input sources as `InputSource = "interactive" | "rpc" | "extension"` (`core/extensions/types.d.ts:588`), and the extension API carries `sendUserMessage(content)` documented verbatim as *"Send a user message to the agent. **Always triggers a turn.**"* An extension loaded with an explicit `--extension` path runs in-process and can call it. **`--no-extensions` does not block an explicit path** — the flag disables *discovery* only, so §12.2's denial of repo-supplied `.pi/extensions/*.ts` is preserved while a pifleet-owned extension still loads. | **Probed, not reasoned.** A container was launched `-d -it` (real pty, `tty=true`) running `pi --no-extensions --extension /probe/probe.ts` in TUI mode (Pi reported `mode=tui hasUI=true`). Nothing was ever typed. A `docker exec` wrote one mounted file; the extension's `fs.watch` fired and called `sendUserMessage`. Pi logged `input source=extension` -> `before_agent_start` -> `agent_start` -> `turn_start` -> `before_provider_request`. **The turn started with zero bytes on the pty.** The expectation in this row's original text — "expect no, a TTY has one owner" — was WRONG, and wrong for an instructive reason: §162 governs who may *write to the terminal*, and this path never touches the terminal at all. | §5.3's deferred auto-trigger is now **available**, and D2's cost paragraph loses most of its force: the keystroke was the whole of what D2 cost the operator, and it can be given back without the §4.3 hazard, because a message submitted through `sendUserMessage` never enters the composer's buffer. **NOT YET PROBED:** whether an operator's half-typed, uncommitted composer line survives an auto-trigger. The one run that reached the composer-occupied case did not deliver its bytes to the pty, so that case is unmeasured — it is the first criterion any build on this finding must carry. |
