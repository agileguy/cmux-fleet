...
/**
 * The identity read moved to `../safety/procstart.ts` so that `safety/kill.ts`
 * could stop importing THIS module to get it — that import was the last live
 * arc of the `kill.ts -> registry.ts -> ... -> reaper.ts -> kill.ts` cycle, and
 * it made `bun test test/unit/kill.test.ts` fail on its own. See that file's
 * header for the measurement. Re-exported here because `registry.ts` is the
 * address every existing caller and doc comment already uses.
 */
export {
  IDENTITY_FORMAT,
  IdentityReadError,
