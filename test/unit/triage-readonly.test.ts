 ...
 * footnote.** `docker/pi-extensions/` holds four extensions running inside the
 * worker container today — `dispatch-trigger.ts`, `truncation-recovery.ts`,
 * `report-tools.ts` since 2026-09-08, and `output-token-cap.ts` since
 * 2026-09-15 — and `tsconfig.json:32` includes only `src/**` and `test/**`, so
 * that directory is outside the typechecker as well as outside this walk.
 * **The third one sharpens the point rather than merely incrementing it:**
 * `report-tools.ts` WRITES the record the host grades a worker by, so the
 * directory this walk cannot see now holds the fleet's only writing verb —
 * the fourth only shapes the request and writes nothing, so that claim
 * still holds.
 *
