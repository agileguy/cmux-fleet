...
const model = (over: Partial<FleetModel> = {}): FleetModel => ({
  runs: ok([{ runId: RUN, models: [], modelsNote: null, workers: [row()] }], NOW - 1_000),
  ...
  test("the ladder renders four distinct cells plus an age, where the incumbent has one word", () => {
    ...
    const rendered = states.map((activity) => {
      const frame = renderFleet(
        model({
          runs: ok([{ runId: RUN, models: [], modelsNote: null, workers: [row({ activity })] }], NOW - 1_000),
        }),
      ).join("\n");
    ...
    const young = renderFleet(
      model({ runs: ok([{ runId: RUN, models: [], modelsNote: null, workers: [row({ activity: "active", transcriptAgeMs: 4_000 })] }], NOW - 1_000) }),
    ).join("\n");
