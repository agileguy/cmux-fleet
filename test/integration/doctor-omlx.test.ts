 ...
 */
function stubOmlx(opts: { completionStatus?: number } = {}): StubOmlx {
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (url.pathname.endsWith("/chat/completions")) {
        ...
        await Bun.sleep(COMPLETION_DELAY_MS);
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        /**
         * MEASURED, not stamped in. The stub holds the response for
         * `COMPLETION_DELAY_MS`, so a value below that floor means the reported
         * figure is not the round trip — a constant, a clock read twice, or a
         * number invented somewhere else would all fail here.
         */
        expect(omlx.completion_latency_ms!).toBeGreaterThanOrEqual(COMPLETION_DELAY_MS);
