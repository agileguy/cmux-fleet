...
- [x] ISC-44: The host `~/.config/gcloud` directory is not in any container's `docker inspect` mount list. Checked at two altitudes: `test/unit/render.test.ts` asserts the ARGV `renderWorker`/`buildDockerArgv` would launch (the exact function `up` calls) never names `HOST_GCLOUD_CONFIG_DIR`, for a `cloud_access: false` role and a `cloud_access: true, adc_mode: file` role alike; `test/integration/adc.test.ts` (Docker-gated) `docker inspect`s three REAL containers built in the three role shapes §5.8 can produce — `cloud_access: false`, `token` mode after a real injection, and `file` mode (built from `fileModeMaterials` against this host's real ADC file) — and asserts no `.Mounts[].Source` is, or is nested under, the host directory, with the one documented exception of the single `HOST_ADC_FILE` `file` mode is allowed to mount. Mutation-proved: adding the forbidden mount to `buildDockerArgv` turns the unit test red; reverted.
- [x] ISC-45: A role with `cloud_access: false` has no Google credential. `test/integration/adc.test.ts` (Docker-gated) starts a container in the exact env `planCredential` returns for such a role (empty — no pointer, no token file, no `/creds` content) and probes both a fresh `env` and `/proc/1/environ` (the two can differ) for all four delivery vectors §5.8 can use (`CLOUDSDK_AUTH_ACCESS_TOKEN_FILE`, `CLOUDSDK_AUTH_ACCESS_TOKEN`, `GOOGLE_OAUTH_ACCESS_TOKEN`, `GOOGLE_APPLICATION_CREDENTIALS`); a same-image, same-probe contrast test on an injected `token`-mode container shows every one of those things present, so the absence is proven to be a property of the role, not of the image.
- [x] ISC-46: In a `cloud_access: false` role, `gcloud auth print-access-token` fails. Run against `/usr/local/libexec/gcloud.real` (the real binary, bypassing the `verbgate` PATH wrapper deliberately — `verbgate` classifies this verb as MUTATING regardless of credential state per ISC-210, a Group J/§5.10 concern orthogonal to this one, and testing through it would measure the gate rather than the credential). Real gcloud's own message is pinned verbatim from a live run ("You do not currently have an active account selected"), not merely a nonzero exit, so the test cannot be satisfied by gcloud being merely missing or gated. Differential proof, live: one container, probed with the same command before and after a REAL token — minted through production `gcloudMinter` from the operator's own already-authenticated ADC, no impersonation, no fake string — is injected; exit flips 1 → 0 and stdout equals the minted token (compared via sha256 digest so a failure never prints a live bearer token). Skips itself (documented, not silently) on a host with no ADC configured.
- [x] ISC-47: After `token_refresh` elapses, a `gcloud` call inside a long-running container still succeeds.
- [x] ISC-48: With `impersonate_service_account` set, the token's identity is the SA, not the launching user's account. Proved at the `up` wiring level in `test/integration/up-wiring.test.ts` (real CLI, real `adc.ts`, PATH-shimmed `gcloud`/`docker`): every worker's grant line names the configured SA and never the shimmed operator account nor the `"(adc user)"` placeholder — substitution, not coexistence — and (the actual security property) the operator's own account is never even asked for: the `gcloud` shim's call log is empty for a run where every `cloud_access` worker impersonates, contrasted against a control run (same fixture, no impersonation) that logs exactly one `config get-value account` call. Mutation-proved: widening `up.ts`'s resolve-identity guard to fire regardless of impersonation leaves every grant line byte-identical and only the call-count assertion goes red — confirming the positive-only checks would not have caught this. **Gap, stated plainly:** this proves pifleet's own code never touches the launching user's identity and correctly threads the SA through; it does not additionally prove, against a real granted service account, that Google's OAuth backend actually mints a token whose own identity (via `tokeninfo`/`auth list`) is the SA. No such SA was available to test against on this host — the SRD's example project (`gen-lang-client-0675968762`) is not reachable under this host's active gcloud identity, and the only real service accounts discoverable belong to a live Broadcom production project; impersonating one for this personal project's test suite was deliberately avoided, and a minimal, read-only `--impersonate-service-account` token mint attempted against one was independently refused by the permission system when tried. Not fixed here; noted rather than silently assumed.
- [x] ISC-49: `up` prints the granted identity, project, and ADC mode for every `cloud_access` worker. Extended in `test/integration/up-wiring.test.ts`: a real `up` run over three workers (two `cloud_access: true` sharing one role, one `cloud_access: false`) with `cloud.adc_mode: file` and a distinctive `cloud.quota_project` asserts every line is keyed by worker id (not merely found), that both cloud workers' lines carry identity + the distinctive project + `"file mode"` (and explicitly not `"token mode"`, so a line ignoring `cloud.adc_mode` is caught), and that the non-cloud worker's line says `"no credential"` and carries none of the three. A companion test confirms the lines reach actual stdout (not only the ledger) when `--json` is absent — the only half an operator running `up` by hand ever sees. The sibling ISC-251 test gained one assertion (`"token mode"` present) so the schema default is exercised too, alongside `file` here.
...
- [ ] ISC-255: A `cloud_access: true` worker's container can actually run `gcloud` — `CLOUDSDK_CONFIG` (`/home/pi/.config/gcloud`) is writable. [FOUND 2026-08-18, Group D close-out. `docker/Dockerfile` bakes `CLOUDSDK_CONFIG=/home/pi/.config/gcloud` as an ordinary directory on the image's root filesystem, but §5.6's real `docker run` flag list is `--read-only` with only `/tmp` mounted as tmpfs — so as launched today, the FIRST `gcloud` call in a credentialed container (either `adc_mode`) crashes: `ERROR: gcloud crashed (OSError): [Errno 30] Read-only file system: '/home/pi/.config/gcloud/configurations'`, measured live on this image with a valid token present. Every `test/integration/adc.test.ts` test that needs `gcloud` to actually run (ISC-44's token/file-mode cases, ISC-46) adds a second tmpfs at that exact path to its own container so the claim under test is observable at all; `buildDockerArgv` itself does not, so a real `pifleet up` worker would hit this the moment ISC-248's wiring lands. Not fixed here: it is a `render.ts`/§5.6 container-runtime-flags change, adjacent to but outside Group D's five assigned criteria, and closing it productively is entangled with ISC-248 (nothing mints/injects a real credential into a real launch yet, so there is no live worker to verify the fix against beyond the same hand-built container shape this session already used). Tracked here so the finding is not lost between phases.]
...
**Progress: 205/256 → 205/257.** No criterion changed state; the denominator moves by one
because ISC-260 was added. Counted from the file rather than incremented by hand:
`grep -c '^- \[x\] ISC-'` = 205, `grep -c '^- \[ \] ISC-'` = 52, 257 total and 257 unique
ids (ISC-248 and ISC-248a are distinct criteria, which is why the id count and the checkbox
count agree). **This line needs reconciliation at merge** — PR #16 and PR #18 are in flight
against the same section of this file and each adds its own criteria, so the TOTAL collides
even though the IDs do not (#16 → 255, this branch → 256 and now 260, #18 → 257/258/259).
Git surfaces that as a text conflict on this line, which is the correct place to resolve it.
The PR body's earlier "210/257" was stale on both halves and has been corrected.
