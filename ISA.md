...
- [x] ISC-186: Registry writes are serialized, so concurrent registrations cannot lose a worker. **[CLOSED 2026-08-19 by this PR.]** The chain in `startRegistryDaemon` was already there; nothing had ever seen it fail. `test/integration/registry-serialization.test.ts` now watches `writeJsonAtomic`'s temp files, which exist for exactly the span of one write to `registry.json`, while twelve registrations arrive concurrently over the real socket: never two at one instant, every worker present on disk afterwards. The obvious test — fire N registrations, assert all N survive — is written up in the file as the trap it is, because it passes with the chain REMOVED: each handler mutates the in-memory registry synchronously and snapshots its payload when its write begins, so a later-starting write always carries a superset and is larger, finishes later, and lands last. Payload size and snapshot recency correlate in the safe direction, so the consequence alone cannot see the race and only the mechanism can. Two vacuity guards stop the sampler passing having watched nothing: it must take more samples than there are writes, and must catch at least half the writes in flight (measured 12 of 12 over 91 samples). Reachability checked rather than assumed, since this ISA already has criteria whose library is correct and unreachable: `up` spawns `daemon` (`src/cli/commands/up.ts:717`), which calls `startRegistryDaemon` (`src/cli/commands/daemon.ts:42`), and supervisors send `register_worker` through it (`src/supervisor/index.ts:177`). Mutation-verified: making `persist()` call `writeJsonAtomic` directly instead of through `serialized()` turns it red with `expect(received).toBe(expected) / Expected: 1 / Received: 12` — all twelve writes in flight at once — reproduced on three consecutive runs, green on three more once restored.
