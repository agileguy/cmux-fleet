...
- [x] ISC-50: A container completes a model call against `host.docker.internal:8000`. **[CLOSED 2026-08-18, egress relay; evidence re-verified 2026-08-19]** Proved by `test/integration/relay.test.ts`, not inferred from the relay existing: a container attached to nothing but the deny-all internal bridge — no `--add-host`, no second network, no capabilities — completes an authenticated `GET http://host.docker.internal:8000/v1/models` against the REAL local oMLX server, and the model ids that come back overlap the ids the host sees on its own direct call. The overlap is the load-bearing part; a relay pointed at some other listener could still return a well-formed model list. The companion stub test carries a full request/response body on an EPHEMERAL port, which is what proves the port is read from `llm.base_url` rather than hardcoded anywhere on the path. **Stated limit:** what the suite asserts is a call to the model SERVER, not an INFERENCE. A real `POST /v1/chat/completions` was measured round-tripping through the relay by hand on 2026-08-19 (705 ms warm, correct answer returned from inside the bridge), but that measurement is deliberately not a test — see ISC-258. **Stated dependency:** the live half of this evidence needs a reachable oMLX and `OMLX_API_KEY`, and that endpoint is currently FLAKY — `com.pai.omlx` SIGABRTed repeatedly on 2026-08-19 under an oversized model load (ISC-258 has the root cause). It was healthy for the run recorded here, and the test self-skips rather than failing when it is not, which means a green suite is NOT by itself proof this criterion re-ran. The stub test — which needs no model server at all — is what holds the relay mechanism unconditionally; the live test is what ties it to the real oMLX.
- [x] ISC-51: That call succeeds with no route to the public internet. **[CLOSED 2026-08-18, egress relay; evidence re-verified 2026-08-19]** Same test, same container, same run as ISC-50, and deliberately so: asserting the reach and the denial against ONE container is what stops a future change from opening the internet in the same edit that fixes the relay and still leaving the suite green. That test also probes the destination BEFORE the relay exists and requires it to fail, so "the call succeeded" cannot be satisfied by an internal network that was never actually internal in the first place.
...
- [~] ISC-57: Egress to any host other than the oMLX endpoint and the configured Google endpoints is denied from inside a container. **[PARTIAL 2026-08-18, egress relay]** The DENY half is closed and live-verified: a container on the internal bridge reaches the oMLX endpoint through the relay and nothing else — `1.1.1.1` fails with curl exit 7 (no route) and `example.com` with exit 6 (no resolver), while the oMLX call in the same container returns 0 with the server's own body. Those exact exit numbers are a HAND MEASUREMENT, re-confirmed 2026-08-19 against the real endpoint; the committed test asserts only that both probes are NON-ZERO. That gap is intentional — pinning curl's exact error numbering would make the suite a hostage to curl's error taxonomy rather than to the network posture it is actually there to check — so the numbers are recorded here as evidence, not asserted there as behaviour. The unclosed half is that the configured Google endpoints are denied TOO, because no relay forwards them (see ISC-253); measured in the same run, `oauth2.googleapis.com` also fails with exit 6. So the live posture is strictly TIGHTER than this criterion describes, not looser; it is marked partial rather than closed because "Google works" is not yet true and must not be implied.
