...
- [ ] ISC-233: Acceptance commands run in a fresh CONTAINER from the same image, not only a fresh clone (SRD §8.2). **STAYS OPEN — audited 2026-08-20. Acceptance runs in a fresh CLONE ON THE HOST and no container is involved at any point, which is exactly the half this criterion's own contrast clause excludes.** `runAcceptance` (`src/harvest/acceptance.ts`) clones by SHA into a scratch dir, checks out detached, and executes each command with `Bun.spawn` inside `execBounded` — the argv is the acceptance command itself, never `docker`. The path is REACHABLE and is the graded one: `harvest/index.ts:313` behind `artifacts --run-acceptance`, and `harvestTask` is called by both `cli/commands/artifacts.ts:106` and `report/collect.ts:308`. So this is not an unreachable-module grade; the production path exists, runs, and simply does not containerize. `AcceptanceContext.image` — the field whose entire purpose is to record which image graded the code — is the literal `null` at `acceptance.ts:556` and has no other writer in the repo. **Both halves were mutation-measured rather than asserted, and they measure opposite.** Mutating `image: null` -> `image: "pifleet/pi-worker:verify"` — an audit record claiming a container that never ran — left `test/unit/acceptance.test.ts` (34), `test/integration/acceptance.test.ts` (11), `test/integration/harvest.test.ts` (34) and `test/unit/adjudicate.test.ts` (25) ALL GREEN: 104 tests, zero failures, no assertion anywhere reads the field (`grep -rln AcceptanceContext test/` returns nothing). The control mutation in the same function proves those tests are not merely weak — running commands in `spec.repo` instead of `cloneDir` turns `test/integration/acceptance.test.ts` RED with 3 failures, on-point ones being "the scratch clone shares no inode with the worker's own checkout" (`Expected: "passed" / Received: "failed"`) and "a harness-touching diff caps a genuinely green run below success" (`Expected: "success" / Received: "failed"`). ISC-149 has teeth; ISC-233 has none. **`[ ]` and not `[~]`, because nothing here is Docker-gated and waiting:** neither acceptance test file mentions `PIFLEET_DOCKER`, both run in the fast `test` job on every push, and the `container` job runs a fixed six-file list (image, verbgate, egress, adc, relay, probe-in-network) that contains no acceptance test. There is no self-skipping probe to un-skip — the behaviour was never built. **What closing it costs, priced rather than guessed.** No reusable "run this argv in a fresh container from image X with dir Y mounted" primitive exists: `container/run.ts` is a subprocess wrapper (`realExec`, `dockerAvailable`) with no container-spawn export; `config/render.ts`'s `buildDockerArgv` is bound to a long-lived worker (`RunPaths`, briefing, `/outbox`, named container); the `mounts.ts` and `image.ts` probes hard-code `--entrypoint /bin/sh -c` and accept no caller argv. The nearest template is `security/probe-transport.ts`'s `containerProbeArgv` (fresh container, pinned image, structured result) which mounts nothing. The argv work is genuinely small — bind-mount the clone with `-w /workspace`, `--entrypoint` past the image's `tini -- pifleet-entrypoint`, export the module-private `RUN_HARDENED` from `image.ts:295`. **Two costs are NOT small, and they are why this is more than a flag change.** (1) The scratch root is `mkdtemp(join(tmpdir(), "pifleet-accept-"))` (`harvest/index.ts:311`), and `mounts.ts` documents BY MEASUREMENT that `os.tmpdir()` is not shared into the macOS Docker VM: it bind-mounts SILENTLY EMPTY. Containerizing naively would grade an empty directory and report passes — strictly worse than today. It must move to `makeDaemonScratch` plus `makeWorkerAccessible` for uid 10001. **That precondition is now tracked on its own as ISC-277 rather than living here as commentary, and the sequencing is the point: the argv work above is small, ISC-277 is not, and doing them in the wrong order ships a containerized acceptance run that grades an empty directory and reports PASSES.** Closing this criterion's argv without ISC-277 is strictly worse than the host-side clone it replaces. (2) No image tag reaches the harvester: `harvestTask` takes `HarvestOptions`, not a config, and `artifacts` is documented to read runs "whose config is long gone" (`acceptance.ts:68`), so the tag must be persisted per-run the way `harness_patterns` already is. If this is wired into CI it belongs in the `container` job, whose hand-maintained `TOTAL_EXPECTED` (79) would need re-deriving by the method that file documents — FLAGGED, not edited here, since another branch is already moving that number.
...
- [ ] ISC-277: The acceptance scratch root is proved daemon-visible BEFORE any acceptance command runs — a host-written sentinel is read back from inside the container, so an unshared path fails loudly instead of grading an empty tree. **FILED OPEN 2026-08-20, split out of ISC-233's audit as a precondition that BLOCKS it rather than a restatement of it.** Someone can finish ISC-233's argv work completely — bind-mount, entrypoint override, hardened flags, all correct — and still ship a containerized acceptance run that grades nothing and certifies the worker. **The mechanism, measured rather than assumed.** `src/container/mounts.ts`'s header records it as a measurement on this machine (Colima 0.9, default profile), not as folklore: `-v` against a path outside the macOS Docker VM's shared set does NOT fail. The daemon creates an empty directory inside the VM and mounts THAT. Its table reads `/var/folders/.../T (os.tmpdir()) → not shared, silently empty`, `/tmp → not shared, silently empty`, `$HOME/... → shared`. **The failure mode is the dangerous kind: not an error, a false pass.** The container reads an empty `/workspace`, the acceptance commands find no tests to fail, they exit 0, and the harvester records `passed` — a green exam against a directory containing nothing. Every symptom points at the worker; the cause is the mount. That is strictly worse than today's host-side clone, which at least grades real files, and it is why this cannot be sequenced after ISC-233's argv. **The gap is concrete and is WIRING, not construction — both remedies already exist, are live, and are exercised in CI.** `makeDaemonScratch` (`mounts.ts:115`) allocates under a daemon-visible root — `$HOME/.pifleet/scratch`, overridable via `PIFLEET_SCRATCH_DIR` — and chmods for uid 10001; `probeMountVisibility` (`mounts.ts:141`) writes a sentinel on the host and reads it back inside the container, which the function's own comment explains is the ONLY thing that separates "shared" from "silently empty", since presence of the mount proves nothing. `probeWriteThrough` round-trips both directions and is already called by `image verify` (`image.ts:369`); `probeMountVisibility` is already called by `doctor` (`doctor.ts:767`); `test/integration/image.test.ts` uses `makeDaemonScratch` and runs inside the `container` job. **What is missing is that the acceptance path uses NONE of them.** `harvest/index.ts:311` is `mkdtemp(join(tmpdir(), "pifleet-accept-"))` — literally `os.tmpdir()`, the exact path `mounts.ts:51` marks "Deliberately NOT `os.tmpdir()`" — and `grep -rn 'makeDaemonScratch\|probeMountVisibility' src/` returns no hit anywhere under `src/harvest/`. So closing this needs the scratch root moved onto `makeDaemonScratch`, plus a visibility probe (or `probeWriteThrough`) gating the run so an unshared root yields `not_run` — which already adjudicates to `unknown` — rather than `passed`. **Correctly `[ ]` and not `[~]`: today the criterion is not merely unproven, its subject does not exist.** Nothing containerizes acceptance yet, so there is no scratch root being checked and no probe to self-skip. It becomes testable the moment ISC-233's argv lands, and the honest test is differential — point the run at a deliberately unshared root and assert the verdict is `unknown` with the mount named, not `success`. Blocks ISC-233; do not sequence it after.
...
- [ ] ISC-243: Anti: the harness surface is a denylist and cannot be complete; a graded allowlist of the resolution surface replaces it. **STAYS OPEN — audited 2026-08-20. The denylist is still the entire mechanism, no allowlist of any kind exists, and the anti-criterion's premise was MEASURED rather than accepted from its own wording.** Naming both things concretely, because the criterion does not: the DENYLIST is `DEFAULT_HARNESS_PATTERNS` (`src/harvest/acceptance.ts:74`), 91 globs, intersected with the diff by `harnessSurface` as `changedFiles.filter(f => globs.some(g => g.match(f)))`. The RESOLUTION SURFACE is the set of files an acceptance command's MEANING routes through — runner config, config inherited by that config, toolchain selection, dependency resolution, and any config naming code that executes before a test does. `adjudicate.ts:216` consumes the result as one boolean, `if (facts.harness.touched.length > 0)`, and nothing else; the only other harness-derived output is ISC-232's `defaults_missed` discrepancy STRING at `:242`, which changes no verdict. Nothing in `HarnessSurfaceSchema` carries a score, weight or tier, so "graded" has no foothold either. **The premise is true, and is now demonstrated on the live matcher rather than argued.** 44 paths probed — 4 controls, 40 resolution-surface: all 4 controls behave (`package.json`, `bunfig.toml`, `.github/workflows/ci.yml` caught; `src/harvest/acceptance.ts` correctly not caught) and 40 of 40 non-control paths are MISSED. `.env`/`.env.test`, `.python-version`, `.ruby-version`, `.sdkmanrc`, `.envrc`, `requirements.txt`, `setup.py`, `Pipfile`, `poetry.lock`, `uv.lock`, `Gemfile`, `Gemfile.lock`, `go.sum`, `Cargo.lock`, `composer.json`, `vite.config.ts` (what vitest reads when no `vitest.config` exists), `babel.config.js`, `.babelrc`, `.swcrc`, `cypress.config.ts`, `ava.config.js`, `nightwatch.conf.js`, the whole Bazel surface (`BUILD.bazel`, `WORKSPACE`, `.bazelrc`), `CMakeLists.txt`, `meson.build`, `build.sbt`, `mix.exs`, `project.clj`, `Directory.Build.props`, `settings.gradle`, `gradle.properties`, `gradle/wrapper/gradle-wrapper.properties`, `Dockerfile`, `docker-compose.yml`. **The shape of the misses is the argument, not their count:** the list matches `go.mod` but not `go.sum`, `Cargo.toml` but not `Cargo.lock`, `pyproject.toml` but not `poetry.lock`, `build.gradle` but not `settings.gradle`, `Rakefile` but not `Gemfile`, and `vitest.config.*` but not `vite.config.*` — six adjacent-sibling holes in a list whose 91 entries were each added by someone thinking hard about that exact family. That is what "cannot be complete" means in practice. **One probed path is honestly weaker than the rest and is recorded as such rather than counted quietly:** `.gitattributes` is missed, but its code-execution route is already closed by ISC-237 — a smudge/diff driver needs a `[filter]`/`[diff]` section in a config git reads, and `git clone` writes the fresh clone's `.git/config` itself rather than copying the worker's, so the driver definition does not survive the hop. What survives is content normalization (`text`/`eol`), which can change what a suite reads without executing anything. Real, but a different and smaller class than the other 39. **The consequence reaches the VERDICT, checked against the real adjudicator rather than inferred from the matcher.** Feeding `adjudicate` a diff of `["src/app.ts", X]` carrying one passing acceptance run: `X = package.json` caps to `unknown` (the control fires), while `X` = `.env.test`, `vite.config.ts`, `requirements.txt`, `.bazelrc`, `babel.config.js`, `.python-version` or `.gitattributes` each certifies `success` with `touched: []`. So the ISC-150 cap is not merely incomplete in principle — a worker can green a red suite through any of them today and be certified. **Deliberately NOT migrated on this branch, and the reason belongs in the record rather than being left implied.** A partial allowlist is strictly worse than a denylist known to be partial: the denylist's failure mode is a miss that reads `success`, consistent with its documented semantics, whereas a half-built allowlist caps on ordinary source files — and the operator's only escape hatch, `harness.patterns`, REPLACES rather than extends, so the first over-cap would be answered by narrowing the surface, which is precisely the silent disable ISC-232 exists to prevent. **What the allowlist would have to enumerate, since that is the deliverable this criterion is actually asking for:** (a) per-runner resolution manifests — for `bun test`, the closure of `bunfig.toml` + `package.json` + `tsconfig*.json` + every `preload` target, and the equivalent for pytest, jest/vitest, go, cargo, gradle and bazel; note this keys the surface on WHICH COMMAND was resolved, which the denylist never was, and the command text is already pinned to the base SHA by ISC-148, so the input exists; (b) the transitive closure of inheritance edges that globs can only approximate (`tsconfig` `extends`, Makefile `include`, `.mocharc` `require`, `jest.config` `setupFiles`, `vitest.workspace`); (c) toolchain pins for every language, not just today's `.nvmrc`/`.tool-versions`/`.mise.toml`; (d) an actual grade — the word implies at least three tiers (executes-before-the-suite, selects-the-toolchain, pins-a-dependency) against today's single boolean, and the all-or-nothing cap is itself what makes narrowing tempting. There is no config surface to hang any of it on: `HarnessSchema` (`src/config/schema.ts:369`) is `.strict()` with the single optional `patterns` key. **One live defect fixed in passing, because it pointed readers away from this criterion:** `test/unit/acceptance.test.ts`'s bypass-regression header said the incompleteness was "recorded as ISC-237". ISC-237 is the `runGit` diff-driver hardening and is CLOSED `[x]`, so the repo's only in-source pointer at this problem sent a reader to a solved, unrelated item — a green-looking cross-reference asserting the question was settled. Corrected to ISC-243 with the misdirection written down so it cannot silently recur. Comment only; no assertion touched, `test/unit/acceptance.test.ts` still 34 pass.
...
- [ ] ISC-282: The stall policy's PRODUCTION inputs and action are covered — `eventSilenceMs` really measures a worker's event silence, and `killWedged` really ends a wedged agent. **FILED OPEN 2026-08-20 by the stall-wiring commission, and filed rather than claimed.** ISC-110 and ISC-117 are `[~]` because `test/unit/scheduler-stall.test.ts` proves the SCHEDULER acts on a verdict, using an injected clock and a fake `eventSilenceMs`. Two production halves are untested: `stat(events.jsonl).mtimeMs` in `dispatch.ts`'s adapter (does an appended event actually move the reading, and does an absent file really answer `null` rather than an infinite silence?), and the `abort` RPC `killWedged` sends (does a wedged supervisor answer it, and does the task then settle?). Closing this needs an integration test with a real supervisor that stops emitting while holding a slot — the same fixture shape ISC-158 already builds.
